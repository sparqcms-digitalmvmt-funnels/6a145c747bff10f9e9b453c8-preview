...
const CHECKOUT_NEXT_PAGE_SLUG = "upsell1";
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "sPPZnbhKMAva7mo_mMw-VhbBpHFlK_KdzYEPEwCVPxM5X8CZ_2EmBsMhLL0lP_Ak",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/6a145c747bff10f9e9b453c8-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
